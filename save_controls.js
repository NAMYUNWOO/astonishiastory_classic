...
const SaveControls = (() => {
  const SLOT_FILES = [
    'ASTO0.SAV', 'ASTO1.SAV', 'ASTO2.SAV', 'ASTO3.SAV', 'ASTO4.SAV',
  ];
  ...
  async function downloadSlot(slotIdx) {
    ...
    const filename0 = SLOT_FILES[slotIdx - 1] || ('ASTO' + (slotIdx - 1) + '.SAV');
    ...
    setTimeout(async function() {
      ...
      try {
        const filename = SLOT_FILES[slotIdx - 1] || ('ASTO' + (slotIdx - 1) + '.SAV');
